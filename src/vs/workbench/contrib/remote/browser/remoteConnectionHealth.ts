...
export class InitialRemoteConnectionHealthContribution implements IWorkbenchContribution {
	...
	private async _checkInitialRemoteConnectionHealth(): Promise<void> {
		try {
			...
			const remoteName = getRemoteName(this._environmentService.remoteAuthority);
			const remoteAiAllowsProvenAbi = Boolean((this.productService as { remoteAiAllowUnsupportedGlibc?: boolean }).remoteAiAllowUnsupportedGlibc) && remoteName === 'ssh-remote';
			if (environment && environment.isUnsupportedGlibc && !remoteAiAllowsProvenAbi) {
